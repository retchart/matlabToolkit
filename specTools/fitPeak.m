function [area,sigma,intercept,slope,xpeak,cfun] = fitPeak(x,y,plotOrNot)
% x 能谱横坐标，道址
% y 能谱纵坐标，每道计数
% area 峰面积，0代表未拟合上
% 拟合公式
...
options.Lower = [1e-5,0.01,originalPeak-5,y(1),min([10*(y(end)-y(1))/(x(end)-x(1)),0])];
%options.Upper = [Inf,Inf,originalPeak+5,Inf,+Inf];
options.Upper = [Inf,100,originalPeak+5,Inf,0];
options.StartPoint = [sum(y)-length(x)*(y(1)+y(end))/2,2,originalPeak,1e3,(y(end)-y(1))/(x(end)-x(1))];
...
if plotOrNot
    ...
    title({'Expression: $y=\frac{A}{\sqrt{2\pi}\sigma}e^{-\frac{(x-\bar{x})^2}{2\sigma^2}}+a+b*x$'; ...
        ...
        ['net area=',num2str(area,'%.1f')]; ...
        ['bkgd area=',num2str(sum(y)-area,'%.1f')]; ...
